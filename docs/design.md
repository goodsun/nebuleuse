...
- [ ] `db.py`：スキーマ初期化（`source` カラムを含む）
- [ ] `embed.py`：multilingual-e5 で埋め込み生成
- [ ] `ingest.py`：素朴な実装（差分検出なし、毎回全更新）
- [ ] `search.py`：vss + FTS5 + RRF（出典情報を必ず返す）
- [ ] `cli.py`：`neb capture` / `neb ingest` / `neb search`
- [ ] 既存の `docs/2026-04-24-*.md` を `neb capture` で取り込んで検索が成立することを確認
